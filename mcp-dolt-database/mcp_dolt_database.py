...
@mcp.tool()
def upload_csv(table_name: str, csv_text: str) -> str:
    """
    Parses CSV text and generates a single multi-row INSERT statement
    to insert data into the specified table.
    Assumes the first row of the CSV is the header.
    Executes the single INSERT statement using the write_query tool.
    Requires the server to be started with an API token.
    """
    if not API_TOKEN:
        return "Error: API token is required for write operations. Please ensure the server was started with the --api-token parameter."

    if not table_name or not csv_text:
        return "Error: Both table_name and csv_text are required."
    ...
    try:
        ...
        header = next(reader)
        if not header:
            return "Error: CSV header row is missing or empty."

        # Prepare column names for the INSERT statement
        column_names = ", ".join([f"`{col.strip()}`" for col in header])

        # Process each data row and collect value strings
        for i, row in enumerate(reader):
            if len(row) != len(header):
                skipped_rows_info.append(f"Row {i+1}: Skipped - Incorrect number of columns (expected {len(header)}, got {len(row)})")
                rows_failed += 1
                continue
            ...
            for val in row:
                ...
                if val_stripped.upper() == 'NULL':
                    values.append("NULL")
                else:
                    try:
                        float(val_stripped) # Check if numeric
                        values.append(val_stripped) # Don't quote numbers
                    except ValueError:
                        # Quote strings, escape internal single quotes
                        escaped_val = val_stripped.replace("'", "''")
                        values.append(f"'{escaped_val}'")

            # Add the formatted tuple string for this row, e.g., "('val1', 123, NULL)"
            all_value_tuples.append(f"({', '.join(values)})")

        # Check if there are any rows to insert
        if not all_value_tuples:
            summary = f"CSV Upload Summary for '{table_name}':\nNo valid data rows found to insert.\n"
            if skipped_rows_info:
                summary += f"Skipped Rows ({rows_failed}):\n" + "\n".join(skipped_rows_info)
            return summary

        # Construct the single multi-row INSERT statement
        values_clause = ",\n  ".join(all_value_tuples)
        sql = f"INSERT INTO `{table_name}` ({column_names}) VALUES \n  {values_clause};"

        # Execute the single statement using write_query
        print(f"[upload_csv] Executing multi-row INSERT for {len(all_value_tuples)} rows.") # Server log
        # print(f"[upload_csv] SQL: {sql}") # Optional: Log the full SQL if needed for debugging

        try:
            result_text = write_query(sql=sql)
            # The result text from write_query for a multi-row insert might just report success or total rows affected.
            final_summary = f"CSV Upload Summary for '{table_name}':\n"
            final_summary += f"Attempted to insert {len(all_value_tuples)} rows in a single statement.\n"
            final_summary += f"Execution Result: {result_text}\n"
            if rows_failed > 0:
                final_summary += f"Skipped Rows ({rows_failed}):\n" + "\n".join(skipped_rows_info)
            return final_summary
        except Exception as e:
            error_msg = f"Error executing multi-row INSERT: {str(e)}"
            print(f"[upload_csv] {error_msg}") # Server log
            # Include skipped rows info even if the main insert fails
            if rows_failed > 0:
                 error_msg += f"\nSkipped Rows ({rows_failed}):\n" + "\n".join(skipped_rows_info)
            return error_msg

    except StopIteration:
        return "Error: CSV text seems empty or contains only a header."
    except Exception as e:
