...
async def _process_single_company(
    ...
    try:
        ...
        # Try to find a filing for the fiscal year
        try:
            selected_filing = None
            
            for filing in filings:
                try:
                    # Try different ways to get the period
                    period_str = None
                    if hasattr(filing, 'period_of_report'):
                        period_str = filing.period_of_report
                    elif hasattr(filing, 'period_end_date'):
                        period_str = filing.period_end_date
                    
                    if period_str and len(str(period_str)) >= 4:
                        period_year = int(str(period_str)[:4])
                        if period_year == year:
                            selected_filing = filing
                            debug_info.append(f"Selected filing with period: {period_str}")
                            break
            ...
            if not selected_filing:
                debug_info.append(f"No matching filing found for FY {year}")
                return f"DEBUG: {'; '.join(debug_info)}"
        ...
        try:
            report_date = full_filing.filing_date if hasattr(full_filing, 'filing_date') else selected_filing.period_of_report
            ...
            debug_info.append(f"Report date: {formatted_date}")
        except Exception as e:
            debug_info.append(f"Error formatting date: {e}")
            formatted_date = f"{year}-12-31"  # fallback
...
('CompanyName', fiscal_year, 'report_date', value1, value2, value3, ...)
